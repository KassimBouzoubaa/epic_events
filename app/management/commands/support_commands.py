...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        read_parser = subparsers.add_parser('read_event', help='Read and filter events by support ID')
        
        # Subparser for update_event
        update_parser = subparsers.add_parser('update_event', help='Update an existing event')
        update_parser.add_argument('event_id', type=int, help='ID of the event to update')
        update_parser.add_argument('--name', type=str, help='Name of the event')
        update_parser.add_argument('--date_de_debut', type=lambda s: datetime.datetime.strptime(s, '%Y-%m-%d').date(), help='Start date of the event in format YYYY-MM-DD')
        update_parser.add_argument('--date_de_fin', type=lambda s: datetime.datetime.strptime(s, '%Y-%m-%d').date(), help='End date of the event in format YYYY-MM-DD')
        update_parser.add_argument('--lieu', type=str, help='Location of the event')
        update_parser.add_argument('--participants', type=int, help='Number of participants')
        update_parser.add_argument('--notes', type=str, help='Notes for the event')
        update_parser.add_argument('--contract_id', type=int, help='ID of the associated contract')
        update_parser.add_argument('--client_id', type=int, help='ID of the associated client')
    ...
    def read_events_by_support_id(self, user, **kwargs):
        try:
            ...
            if not events.exists():
                self.stdout.write(self.style.WARNING(f'No events found for support with ID {user.id}'))
                return
            ...
            self.stdout.write(self.style.ERROR(f'Error reading events: {e}'))
    ...
    def update_event(self, user, **kwargs):
        ...
        try:
            ...
            if event.support.id != user.id:
                    self.stdout.write(self.style.ERROR(f'You are not responsible for the event'))
                    return
