...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        create_client_parser = subparsers.add_parser('create_client', help='Create a new client')
        create_client_parser.add_argument('information', type=str, help='Information about the client')
        create_client_parser.add_argument('nom', type=str, help='Last name of the client')
        create_client_parser.add_argument('prenom', type=str, help='First name of the client')
        create_client_parser.add_argument('email', type=str, help='Email of the client')
        create_client_parser.add_argument('telephone', type=str, help='Phone number of the client')
        create_client_parser.add_argument('nom_entreprise', type=str, help='Company name of the client')
        create_client_parser.add_argument('commercial_id', type=int, help='ID of the commercial collaborator')

        # Subparser for update_client
        update_client_parser = subparsers.add_parser('update_client', help='Update an existing client')
        update_client_parser.add_argument('client_id', type=int, help='ID of the client to update')
        update_client_parser.add_argument('--information', type=str, help='New information about the client')
        update_client_parser.add_argument('--nom', type=str, help='New last name of the client')
        update_client_parser.add_argument('--prenom', type=str, help='New first name of the client')
        update_client_parser.add_argument('--email', type=str, help='New email of the client')
        update_client_parser.add_argument('--telephone', type=str, help='New phone number of the client')
        update_client_parser.add_argument('--nom_entreprise', type=str, help='New company name of the client')
        update_client_parser.add_argument('--commercial_id', type=int, help='New ID of the commercial collaborator')
        
        # Subparser for read_contract
        read_contract_parser = subparsers.add_parser('read_contract', help='Read and filter contracts')
        read_contract_parser.add_argument('--client_id', type=int, help='Filter contracts by client ID')
        read_contract_parser.add_argument('--commercial', action='store_true', help='Filter contracts by commercial ID')
        read_contract_parser.add_argument('--unpaid_only', action='store_true', help='Filter contracts that are not fully paid')
        read_contract_parser.add_argument('--inactive_only', action='store_true', help='Filter inactive contracts')
        ...
        update_contract_parser.add_argument('--signed', action='store_true', help='Set the contract status to signed')
        update_contract_parser.add_argument('--not_signed', action='store_true', help='Set the contract status to not signed')
        update_contract_parser.add_argument('--client_id', type=int, help='Set the client for the contract')

        
        # Subparser for create_event
        create_event_parser = subparsers.add_parser('create_event', help='Create a new event')
        create_event_parser.add_argument('name', type=str, help='Name of the event')
        create_event_parser.add_argument('date_de_debut', type=lambda s: datetime.datetime.strptime(s, '%Y-%m-%d').date(), help='Start date of the event in format YYYY-MM-DD')
        create_event_parser.add_argument('date_de_fin', type=lambda s: datetime.datetime.strptime(s, '%Y-%m-%d').date(), help='End date of the event in format YYYY-MM-DD')
        create_event_parser.add_argument('lieu', type=str, help='Location of the event')
        create_event_parser.add_argument('participants', type=int, help='Number of participants')
        create_event_parser.add_argument('notes', type=str, help='Notes for the event')
        create_event_parser.add_argument('support_id', type=int, help='ID of the supporting collaborator')
        create_event_parser.add_argument('contract_id', type=int, help='ID of the associated contract')
    ...
    def create_client(self, user, **kwargs):
        ...
        try:
            ...
            client.save()
            self.stdout.write(self.style.SUCCESS(f'Client {client.nom_complet} created successfully'))
        except Collaborateur.DoesNotExist:
            self.stdout.write(self.style.ERROR(f'Collaborateur with id {commercial_id} does not exist'))
    ...
    def update_client(self, user, **kwargs):
        ...
        try:
            ...
            if client.commercial.id != user.id:
                self.stdout.write(self.style.ERROR(f'You are not responsible of this client'))
                return
            ...
            client.save()
            self.stdout.write(self.style.SUCCESS(f'Client {client.nom_complet} updated successfully'))
    ...
    def create_event(self,user, **kwargs):
        ...
        try:
            ...
            if not contract.statut:
                self.stdout.write(self.style.ERROR(f'Cannot create event for an unsigned contract (ID: {contract_id}).'))
                return
            ...
            event.save()
            self.stdout.write(self.style.SUCCESS(f'Event {event.name} created successfully'))
        except Contract.DoesNotExist:
            self.stdout.write(self.style.ERROR(f'Contract with id {contract_id} does not exist'))
        except Client.DoesNotExist:
            self.stdout.write(self.style.ERROR(f'Client with id {client_id} does not exist'))
        except Collaborateur.DoesNotExist:
            self.stdout.write(self.style.ERROR(f'Collaborateur with id {support_id} does not exist'))