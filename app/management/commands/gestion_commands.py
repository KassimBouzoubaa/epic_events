...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        create_collaborateur_parser = subparsers.add_parser(
            "create_collaborateur", help="Create a new collaborateur"
        )
        create_collaborateur_parser.add_argument(
            "nom", type=str, help="Last name of the collaborateur"
        )
        create_collaborateur_parser.add_argument(
            "prenom", type=str, help="First name of the collaborateur"
        )
        create_collaborateur_parser.add_argument(
            "username", type=str, help="Username of the collaborateur"
        )
        create_collaborateur_parser.add_argument(
            "email", type=str, help="Email of the collaborateur"
        )
        create_collaborateur_parser.add_argument(
            "role_name", type=str, help="Role name of the collaborateur"
        )
        create_collaborateur_parser.add_argument(
            "password", type=str, help="Password of the collaborateur"
        )

        # Subparser for update_collaborateur
        update_collaborateur_parser = subparsers.add_parser(
            "update_collaborateur", help="Update an existing collaborateur"
        )
        update_collaborateur_parser.add_argument(
            "collaborateur_id", type=int, help="ID of the collaborateur to update"
        )
        update_collaborateur_parser.add_argument(
            "--nom", type=str, help="New last name of the collaborateur"
        )
        update_collaborateur_parser.add_argument(
            "--prenom", type=str, help="New first name of the collaborateur"
        )
        update_collaborateur_parser.add_argument(
            "--email", type=str, help="New email name of the collaborateur"
        )
        update_collaborateur_parser.add_argument(
            "--role_name", type=str, help="New role of the collaborateur"
        )

        # Subparser for read_collaborateur
        read_collaborateur_parser = subparsers.add_parser(
            "read_collaborateur", help="Read and filter collaborateur"
        )
        read_collaborateur_parser.add_argument(
            "--role_name", type=str, help="Filter collaborateurs by role name"
        )

        # Subparser for delete_collaborateur
        delete_collaborateur_parser = subparsers.add_parser(
            "delete_collaborateur", help="Delete collaborateur"
        )
        delete_collaborateur_parser.add_argument(
            "collaborateur_id", type=int, help="ID of the collaborateur to delete"
        )

        # Subparser for create_contract
        create_contract_parser = subparsers.add_parser(
            "create_contract", help="Create a new contract"
        )
        create_contract_parser.add_argument(
            "montant_total", type=int, help="Total amount of the contract"
        )
        create_contract_parser.add_argument(
            "client_id", type=int, help="ID of the client associated with the contract"
        )
        create_contract_parser.add_argument(
            "commercial_id",
            type=int,
            help="ID of the commercial collaborator associated with the contract",
        )
        ...
        update_contract_parser.add_argument(
            "--signed", action="store_true", help="Set the contract statut to true"
        )
        update_contract_parser.add_argument(
            "--not_signed", action="store_true", help="Set the contract statut to false"
        )
        update_contract_parser.add_argument(
            "--client_id", type=int, help="ID of the associated client"
        )
        update_contract_parser.add_argument(
            "--commercial_id", type=int, help="ID of the associated commercial"
        )

        # Subparser for read_event_without_support
        read_event_parser = subparsers.add_parser(
            "read_event", help="Read and filter events without support"
        )
        ...
        update_event_parser.add_argument(
            "--lieu", type=str, help="Location of the event"
        )
        update_event_parser.add_argument(
            "--participants", type=int, help="Number of participants"
        )
        update_event_parser.add_argument(
            "--notes", type=str, help="Notes for the event"
        )
        update_event_parser.add_argument(
    ...
    def create_collaborateur(self, **kwargs):
        ...
        try:
            ...
            collaborateur = Collaborateur.objects.create(
                nom=nom, prenom=prenom, email=email, role=role, username=username
            )
            collaborateur.set_password(password)
            collaborateur.save()
            self.stdout.write(
                self.style.SUCCESS(
                    f"Collaborateur {collaborateur.nom_complet} created successfully"
                )
            )

            log_event_collaborateur("Collaborateur created", collaborateur)
    ...
    def update_collaborateur(self, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Collaborateur {collaborateur.nom_complet} updated successfully"
                )
            )

            log_event_collaborateur("Collaborateur updated", collaborateur)
    ...
    def delete_collaborateur(self, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Collaborateur {collaborateur.nom_complet} deleted successfully"
                )
            )
    ...
    def create_contract(self, **kwargs):
        ...
        try:
            ...
            log_event_contract("Contract created", contract)
